...
impl CausalReplayEngine {
    ...
    pub fn record_event(&mut self, event: CausalEvent) {
        if self.is_replaying { return; } // Read-only during replay
    ...
    fn cleanup(&mut self) {
        let now = Instant::now();
        let cutoff = now.checked_sub(std::time::Duration::from_secs_f32(self.max_duration_secs))
            .unwrap_or(now);
    ...
    pub fn get_replay_data(&self) -> Option<CausalSnapshot> {
        if !self.is_replaying { return None; }
    ...
    pub fn get_events_near(&self, window_secs: f32) -> Vec<&CausalEvent> {
        ...
        self.event_log.iter()
            .filter(|(t, _)| {
                let diff = if *t > target_time { *t - target_time } else { target_time - *t };
                diff < half_window
            })
            .map(|(_, e)| e)
            .collect()
