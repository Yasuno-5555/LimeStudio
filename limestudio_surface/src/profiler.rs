...
impl FrameProfiler {
    ...
    pub fn end_frame(&mut self) {
        ...
        self.frames
            .push(self.last_frame_time.as_secs_f32() * 1000.0);
    }
    ...
    pub fn report(&self) {
        ...
        println!(
            "  UI Logic:    {:.2}ms",
            self.ui_logic_time.as_secs_f32() * 1000.0
        );
        println!(
            "  Text Layout: {:.2}ms",
            self.text_layout_time.as_secs_f32() * 1000.0
        );
        println!(
            "  GPU Render:  {:.2}ms",
            self.gpu_render_time.as_secs_f32() * 1000.0
        );
        println!(
            "  Total Frame: {:.2}ms",
