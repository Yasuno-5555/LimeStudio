...
use limestudio_surface::ui_ir::{SurfaceWidget, SurfaceId, FrameStyle, TreeNode};
...
impl SqueezerApp {
    ...
    fn build_ide_ui(&mut self) -> SurfaceWidget {
            ...
            for p in packets {
                ...
                match p.event {
                    ...
                    limestudio_core::telemetry::TelemetryEvent::NanDetected { .. } => {
                        self.terminal_history.push("[TELEMETRY] CRITICAL: NaN DETECTED".to_string());
                    }
                    _ => {}
                }
            }
        ...
        while let Ok(timed_event) = self.event_consumer.pop() {
            match timed_event.event {
                SurfaceEvent::PointerDown { position, button, modifiers } => {
                    if self.is_design_mode {
                        self.designer.handle_event(&timed_event.event);
                    } else {
                        self.vpl_engine.handle_event(SurfaceEvent::PointerDown { position, button, modifiers });
                    }
                }
                SurfaceEvent::PointerUp { position, button, modifiers } => {
                    if self.is_design_mode {
                        self.designer.handle_event(&timed_event.event);
                    } else {
                        self.vpl_engine.handle_event(SurfaceEvent::PointerUp { position, button, modifiers });
                    }
                }
        ...
        let packed = self.move_state.packed.load(std::sync::atomic::Ordering::Acquire);
        if packed != 0 {
        ...
        let top_bar = SurfaceWidget::Box {
            ...
            children: vec![
                SurfaceWidget::Row {
                    children: vec![
                        SurfaceWidget::Label { text: format!("SQUEEZER - {}", self.project.name), is_secondary: false },
                        SurfaceWidget::Button { id: SurfaceId::from_seed("ide_new"), label: "NEW".to_string(), is_active: false },
                        SurfaceWidget::Button { id: SurfaceId::from_seed("ide_open"), label: "OPEN".to_string(), is_active: false },
                        SurfaceWidget::Button { id: SurfaceId::from_seed("ide_save"), label: "SAVE".to_string(), is_active: false },
                ...
                SurfaceWidget::TreeView {
                    ...
                    nodes: vec![
                        ...
                        TreeNode {
                            ...
                            children: vec![
                                TreeNode { id: SurfaceId::from_seed("p_cutoff"), label: "Cutoff".to_string(), children: vec![], is_expanded: false },
                                TreeNode { id: SurfaceId::from_seed("p_res"), label: "Resonance".to_string(), children: vec![], is_expanded: false },
                            ],
                            is_expanded: true,
                        },
                    ],
        ...
        let main_view = SurfaceWidget::Box {
            style: if self.is_design_mode { FrameStyle::Standard } else { FrameStyle::None },
            children: vec![
                if self.is_design_mode {
                ...
                SurfaceWidget::Column {
                    children: vec![
                        ...
                        SurfaceWidget::DataTable {
                            ...
                            columns: vec!["TIMESTAMP".to_string(), "EVENT".to_string(), "TARGET".to_string()],
                            rows: self.build_log.iter().map(|s| vec![limestudio_core::time::Timestamp::now().to_string(), "BUILD_LOG".to_string(), s.clone()]).collect(),
                        }
                    ]
                }
            ]
    ...
    fn _export_ui_code(&mut self) {
        ...
        self.terminal_history.push("--- EXPORTED UI CODE ---".to_string());
    ...
    fn _trigger_build_with_target(&mut self, target: &str) {
        ...
        let orchestrator = BuildOrchestrator::new(self.project.name.clone(), "dev.limestudio.plugin".to_string());
    ...
    fn _run_command(&mut self, cmd: &str) {
        ...
        match args[0] {
            "help" => {
                self.terminal_history.push("Available: build [mac|ios|win|linux], doctor, verify, clear".to_string());
            }
            "clear" => {
            ...
            "build" => {
                ...
                self.terminal_history.push(format!("Triggering build for {}...", target));
                self._trigger_build_with_target(target);
            }
            "doctor" => {
                self.terminal_history.push("Running LimeDoctor...".to_string());
                self.terminal_history.push("Result: READY FOR PRODUCTION".to_string());
            }
            _ => {
                self.terminal_history.push(format!("Unknown command: {}", args[0]));
            }
        }
...
impl ApplicationHandler for SqueezerApp {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        let window = Arc::new(event_loop.create_window(Window::default_attributes().with_title("Squeezer IDE")).unwrap());
    ...
    fn window_event(&mut self, event_loop: &ActiveEventLoop, _id: WindowId, event: WindowEvent) {
        match event {
            ...
            WindowEvent::Resized(new_size) => {
                self.bridge.push(SurfaceEvent::Resize { width: new_size.width, height: new_size.height });
            }
