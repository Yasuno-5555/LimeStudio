...
impl WaveletEngineWrapper {
    pub fn new(sample_rate: f64, num_scales: usize) -> Self {
        ...
        let processor = WaveletProcessor::new(sample_rate, fft_size, hop_size, num_scales, &morlet);
    ...
    pub fn process(&mut self, input: &[f32], output: &mut [f32]) {
        ...
        self.gatherer
            .process_stream(input, output, |in_block, out_block| {
                // ここで固定長の処理が走る
...
impl AudioProcessor for WaveletEngineWrapper {
    fn prepare(&mut self, context: &ProcessContext) {
        ...
        if (self.sample_rate - context.sample_rate).abs() > 0.1 {
            ...
            let new_proc = WaveletProcessor::new(self.sample_rate, fft_size, hop_size, 5, &morlet);
